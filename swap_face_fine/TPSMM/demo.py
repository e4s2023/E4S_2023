...
import torch
from swap_face_fine.TPSMM.modules.inpainting_network import InpaintingNetwork
from swap_face_fine.TPSMM.modules.keypoint_detector import KPDetector
from swap_face_fine.TPSMM.modules.dense_motion import DenseMotionNetwork
from swap_face_fine.TPSMM.modules.avd_network import AVDNetwork
...
def drive_source_demo(source_img, target_img, cfg_path, ckpt_path):
    
    driving_video = [target_img]
    inpainting, kp_detector, dense_motion_network, avd_network = load_checkpoints(config_path=cfg_path, checkpoint_path=ckpt_path, device=torch.device('cuda'))
 
    predictions = make_animation(source_img, 
                                    driving_video, 
                                    inpainting, 
                                    kp_detector, 
...
if __name__ == "__main__":
    ...
    target_image = imageio.imread(opt.target_image)
    """
    ...
    target_image = resize(target_image, opt.img_shape)[..., :3]
    # driving_video = [resize(frame, opt.img_shape)[..., :3] for frame in driving_video]
    driving_video = [target_image]
    ...
    imageio.imsave('res.png', predictions[0])
