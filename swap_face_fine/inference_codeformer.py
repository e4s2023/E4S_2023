...
def face_sr(face_img, w=0.8, upscale=2):
    ...
    restored_face = face_upsampler.enhance(restored_face, outscale=upscale)[0]
    restored_face = restored_face[:, :, ::-1].astype('uint8')

    return Image.fromarray(restored_face)


class CodeFormerInfer(object):
    def __init__(self):
