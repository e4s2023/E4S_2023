...
class GCFSR(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_body_first = ConvLayer(4, channels[f'{first_out_size}'], 3, bias=True, activate=True)
        # downsample
        ...
        self.final_linear = EqualLinear(512, self.num_style_feat * self.num_latent, bias=True, activation='fused_lrelu')
        ...
        for i in range(self.log_size, 2+1, -1):
            ...
            self.condition_scale1.append(
                EqualLinear(512, out_channels, bias=True, bias_init_val=1, activation=None))
    ...
    def forward(self, x, cond,
        ...
        scale1 = self.condition_scale1[0](cond)
        scales1.append(scale1.clone())
        scale2 = self.condition_scale2[0](cond)
        ...
        for i in range(len(self.conv_body_down)):
            ...
            if j < len(self.condition_scale1):
                scale1 = self.condition_scale1[j](cond)
                scales1.append(scale1.clone())
                scale2 = self.condition_scale2[j](cond)
                scales2.append(scale2.clone())
        ...
        latent = self.final_linear(cond).view(-1, self.num_latent, self.num_style_feat)
        ...
        if return_latents:
            ...
            return image, None, scales1[1], scales2[1]



@ARCH_REGISTRY.register()
class ColorizationArch(nn.Module):
    ...
    def __init__(self,
                 ...
                 resample_kernel=(1, 3, 3, 1),
                 narrow=1):
        super(ColorizationArch, self).__init__()
        ...
        self.conv_body_first = ConvLayer(1, channels[f'{first_out_size}'], 3, bias=True, activate=True)
        # downsample
        in_channels = channels[f'{first_out_size}']
        self.conv_body_down = nn.ModuleList()
        for i in range(self.log_size-1, 2+1, -1):
        ...
        self.out_conv = ConvLayer(3, 2, kernel_size=3, activate=False)
...
@ARCH_REGISTRY.register()
class SuperResolutionArch(nn.Module):
    ...
    def __init__(self,
                 ...
                 resample_kernel=(1, 3, 3, 1),
                 narrow=1):
        super(SuperResolutionArch, self).__init__()
...
@ARCH_REGISTRY.register()
class FaceInpaintingArch(nn.Module):
    ...
    def __init__(self,
                 ...
                 resample_kernel=(1, 3, 3, 1),
                 narrow=1):
        super(FaceInpaintingArch, self).__init__()
