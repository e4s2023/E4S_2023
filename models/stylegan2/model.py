...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style):
        ...
        )  # 把bs归到一起，因为一个batch中的每个sample都有不同的style，自然mod/demod的参数都不一样，相当于把一个batch里面所有图片的所有通道拼接起来，看成一整张“很长”的图片
        ...
        if self.upsample:
            ...
            _, _, height, width = out.shape
            out = out.view(batch, self.out_channel, height, width)  # 然后将bs变回来
...
class NoiseInjection(nn.Module):
    ...
    def forward(self, image, noise=None):
        if noise is None:
            ...
            # 每个pixel加不同的噪声，但是相同pixel的所有通道噪声一样
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, mask, noise=None):
        if not self.mask_op:
            ...
            # ============= 修改 ========================
            # 对每个component做 style modulation 和 demodulation
            bs, _, H, W = input.size()
            ...
            num_comp = style.size(1)  # [bs,9,512]
        ...
        # num_comp = style.size(1)  # [bs,9,512], num_comp=9
        # for comp_i in range(num_comp):
        #     out_i = self.conv(input, style[:, comp_i, :])
            
        #     mask_i = segmap[:, comp_i, :, :].unsqueeze(dim=1).bool().repeat(1,self.conv.out_channel,1,1)
        #     comp_aware_out.masked_scatter_(mask_i, out_i[mask_i])
        # # ============= 修改 ========================
        
        # TODO: 这个noise是放到 per-component里面还是外面？
...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, mask, skip=None):
        if not self.mask_op:
            ...
            # ============= 修改 ========================
            # 对每个component做 style modulation 和 demodulation,
            # 此时必须保证每个comp都有一个style code，即style的shape是[bs, #seg_cls, 512]
            bs, _, H, W = input.size()
            segmap = F.interpolate(mask, size=(H,W), mode='nearest') # one-hot格式, [bs,#seg_cls,H,W]
        ...
        #     mask_i = segmap[:, comp_i, :, :].unsqueeze(dim=1).bool().repeat(1,3,1,1)
        #     comp_aware_out.masked_scatter_(mask_i, out_i[mask_i])
        # # ============= 修改 ========================
